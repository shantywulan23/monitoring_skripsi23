<ul class="navbar-nav sidebar sidebar-light accordion" id="accordionSidebar">
    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{ route('dashboard') }}">
        <div class="sidebar-brand-icon">
            <img src="{{ url('logounmus.png') }}">
        </div>
        <div class="sidebar-brand-text">Sistem Bimbingan Skripsi</div>
    </a>
    ...
    @if (Auth::user()->role === 'ADMIN')
        <li class="nav-item @if (Route::is('data-dosen.*') || Route::is('data-mahasiswa.*')) active @endif">
            ...
            <div id="collapseBootstrap" class="@if (Route::is('data-dosen.*') || Route::is('data-mahasiswa.*')) show active @endif collapse"
                aria-labelledby="headingBootstrap" data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    <h6 class="collapse-header">Kelola Data</h6>
                    <a class="collapse-item @if (Route::is('data-mahasiswa.*')) active @endif"
                        href="{{ route('data-mahasiswa.index') }}">Data Mahasiswa</a>
                    <a class="collapse-item @if (Route::is('data-dosen.*')) active @endif"
                        href="{{ route('data-dosen.index') }}">Data Dosen</a>
                </div>
            </div>
        </li>
    ...
    @if (Auth::user()->role === 'MAHASISWA' && Auth::user()->dosen_utama != null && Auth::user()->dosen_pendamping != null)
        @if (Auth::user()->role === 'MAHASISWA')
            <li class="nav-item @if (Route::is('bimbingan.show_pembimbing_utama') ||
                    Route::is('bimbingan.show_pembimbing_pendamping') ||
                    Route::is('bimbingan.riwayat-bimbingan')) active @endif">
                <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseBootstrap"
                    aria-expanded="true" aria-controls="collapseBootstrap">
                    <i class="far fa-fw fa-window-maximize"></i>
                    <span>Bimbingan Skripsi</span>
                </a>
                <div id="collapseBootstrap" class="@if (Route::is('bimbingan.show_pembimbing_utama') ||
                        Route::is('bimbingan.show_pembimbing_pendamping') ||
                        Route::is('bimbingan.riwayat-bimbingan')) show active @endif collapse"
                    aria-labelledby="headingBootstrap" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <h6 class="collapse-header">Bimbingan Skripsi</h6>
                        <a class="collapse-item @if (Route::is('bimbingan.show_pembimbing_utama')) active @endif"
                            href="{{ route('bimbingan.show_pembimbing_utama') }}">Pembimbing 1 (PU)</a>
                        <a class="collapse-item @if (Route::is('bimbingan.show_pembimbing_pendamping')) active @endif"
                            href="{{ route('bimbingan.show_pembimbing_pendamping') }}">Pembimbing 2 (PP)</a>
                        <a class="collapse-item @if (Route::is('bimbingan.riwayat-bimbingan')) active @endif"
                            href="{{ route('bimbingan.riwayat-bimbingan') }}">Riwayat Bimbingan</a>
                    </div>
                </div>
            </li>
